    ...
    <div id="single=column">
        ...
        <tr>
            ...
            $form = new Form();
            $form->open($config['fantasy_web_root'].'league/leagueContact/','detailsForm|detailsForm');
            $form->fieldset('');
            $form->text('name','Name','required|trim',$this->input->post('name'),array('class'=>'longtext'));
            $form->space();
			$form->text('email','E-mail Address (optional)','trim|valid_email',$this->input->post('email'),array('class'=>'longtext'));
            $form->space();
