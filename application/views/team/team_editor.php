    ...
    $(document).ready(function(){		   
		$('#delete').click(function(){
			document.location.href = '<?php echo($config['fantasy_web_root']); ?>team/delete/<?php echo($thisItem['id']); ?>';
		});	
		$('#cancel').click(function(){
			<?php if (isset($thisItem['id']) && ($thisItem['id'] != "add" && $thisItem['id'] != -1)) { ?>
			document.location.href = '<?php echo($config['fantasy_web_root']); ?>team/admin/<?php echo($thisItem['id']); ?>';
			<?php } else { ?>
			history.back(-1);
			<?php } ?>
		});
    ...
    <div id="center-column">
        ...
        <table class="listing form" cellpadding="0" cellspacing="0">
          <tr>
            <th class="full" colspan="2">Enter the information for this team below.</th>
          </tr>
          <tr>
